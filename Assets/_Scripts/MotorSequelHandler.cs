...
public class MotorSequelHandler : MonoBehaviour
{
    ...
    [SerializeField] private MonoBehaviour playerControllerToDisable; //Puse esto para no modificar el script del player controller y no de error
    ...
    void UpdateFeedback(bool k1, bool k2, bool k3, bool cursorOk)
    {
        if (progressBar != null)
        {
            progressBar.maxValue = requiredHoldTime;
            progressBar.value = holdTimer;
        }

        if (statusText != null)
        {
            float restante = Mathf.Max(0f, requiredHoldTime - holdTimer);
            ...
            statusText.text =
                $"{sShift}   {sG}   {sN}\n" +
                $"{sCursor}\n" +
                $"Tiempo restante: {restante:0.0}s";
        }
    }
